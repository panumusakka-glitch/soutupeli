...
run(`resetBotRacers();const bot=botRacers[0];const requested=criticalStrokePower(bot.rower)+10;bot.stamina=100;updateBotWPrime(bot,900,requested);if(bot.stamina!==0||effectiveBotPower(bot,requested)!==criticalStrokePower(bot.rower))throw Error('Bot W prime');`);
run(`if(rowerSweatRate(rowers.find(r=>r.name==='Panu Musakka'))<=rowerSweatRate(rowers.find(r=>r.name==='Hanna Tuominen')))throw Error('Sweat profiles');const day=randomRaceDay(rower);if(!Number.isFinite(day.heat))throw Error('Race-day heat');`);
run(`rower=rowers.find(r=>r.name==='Panu Musakka');hydration=energy=100;sodiumBalance=700;cramps=0;stamina=10;strokePower=110;lastPowerSetting=70;powerSurge=0;updateCramps(60,1,true,{wind:1,speedLoss:.85});const hardCrampGrowth=cramps;cramps=0;stamina=100;strokePower=60;lastPowerSetting=60;powerSurge=0;updateCramps(60,.4,true,{wind:0,speedLoss:0});if(hardCrampGrowth<=cramps)throw Error('Cramp load');`);
run(`techniqueControl=1;energy=20;hydration=55;stamina=10;strokePower=110;updateTechniqueControl(3600,{wind:1},1,true);if(techniqueControl>=1)throw Error('Technique fatigue');`);
