...
function createGame(directory = root, legacy = false) {
  ...
  const box={console,document:{getElementById:id=>{assert(nodes[id],`Missing element ${id}`);return nodes[id]},createElement:node,
    ...
    rowingAudio:{stop:noop,stopCrowd:noop,cheerStart:noop,unlock:noop,startMenuMusic:noop,starterShot:noop,catchOar:noop,release:noop,isEnabled:()=>true},Image:function(){},Option:function(){},
